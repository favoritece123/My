...
setInterval(() => {
  http.get(`https://fuckeveryone88tatt9ccckt.glitch.me/`);
}, 280000);
...
client.on("ready", () => {
  ...
  client.user.setActivity("Playing Forget", {
    type: "STREAMING",
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "new")) {
    const reason = message.content
      ...
      .slice(1)
      .join(" ");
    if (!message.guild.roles.exists(gg => gg.name === "Soul,"))
    ...
    message.guild
      ...
      .then(c => {
        let role = message.guild.roles.find(gg => gg.name === "Soul,");
    ...
    if (
      !message.member.roles.has(
        message.guild.roles.filter(r => r.name === "Soul,").first().id
      )
    )
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@728764805510594613>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "هلا") {
    message.channel.send("**ي هلا منورنا**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "برب") {
    message.channel.send("**تيت لاتطول**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "باك") {
    message.channel.send("**ولكم نورتنا**");
    message.channel.sendFile("");
  }
});
////تعديل غير اساسي
        ...
        .then(collected => {
          ...
          message.channel
            ...
            .then(async collectedd => {
              ...
              let steve = await client.fetchUser("712622097372807168");
              embed1.setFooter(
                `رد تلقائي`,
...
client.on("message", async message => {
  ...
  if (args[0].toLowerCase() == `${prefix}credits`) {
    ...
    if (!args[2]) {
      message.channel.send(
        `**:moneybag: |${mention.username}, your account balance is \`$${credits[mention.id].credits}.\`**`
      );
